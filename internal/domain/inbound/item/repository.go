...
func (r *Repository) Get(id Id) (*Model, error) {
	data, err := sqlboiler.FindInboundItem(context.Background(), r.Db, id.ToString())
	if err != nil {
		return &Model{}, err
	}
...
func (r *Repository) Find(id Id) (bool, error) {
	found, err := sqlboiler.InboundItemExists(context.Background(), r.Db, id.ToString())
	if err != nil {
		return false, err
	}
